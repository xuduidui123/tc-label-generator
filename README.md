...
## 功能
- **一张合并数据源**：外箱/内盒/ITF/BD 四类标签共用同一张 Excel，只填一次。
- 上传后勾选本次要生成的类型，「一键生成」，输出按类型分子文件夹的打包 ZIP。
- **每行开关列**逐行控制某产品是否生成外箱/内盒/ITF-UK/ITF-CE/BD。
- **自动完成**：五类文件命名、CON（不加粗+加粗）、外箱尺寸（长×宽×高）、UK/CE 分支（PO 号判断）均由程序生成，无需手填。
- **生成前体检**：跑之前先列出隐患（缺列、ITF 奇数位、条码与目视号不符、命名重复）。
- 外箱标按 PO 号自动区分 UK/爱尔兰（380/520）与 CE（999）两种版式。
- ITF / BD 条码由数据自动生成，并对条码值做一致性校验；生成后给首张预览。
